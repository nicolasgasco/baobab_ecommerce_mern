...
const userSchemaJoi = Joi.object({
  ...
  birthday: Joi.date()
    .max(`${new Date().getMonth() + 1}-${new Date().getDate()}-${new Date().getFullYear() - 18}`)
    .min(`${new Date().getMonth() + 1}-${new Date().getDate()}-${new Date().getFullYear() - 125}`)
    .required(),
  ...
  address: {
    ...
    street: Joi.string().trim().min(2).max(25),
